...
main()
{
	int a[100],b[100],n1,n2,i,R[100],j,t,total;
	printf("Input number of elements in the first array\n");
	scanf("%d",&n1);//n1 will be the number of the element   kept in the first array
	printf("Input %d numbers\n",n1);
	for(i=0;i<n1;i++){
		scanf("%d",&a[i]);
	}
	printf("Input numbers of elements in second array\n");
	scanf("%d",&n2);//n2 will be the number of the element   kept in the second array
	printf("Input %d numbers\n",n2);
	for(i=0;i<n2;i++){
		scanf("%d",&b[i]);
	}
	...
	total=n1+n2;//this is the total number of elements in the arrays combined
	for(i=0;i< total;i++){
		for(j=0;j<i;j++){
